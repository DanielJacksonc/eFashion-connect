...
$(document).ready(function() {
            ...
            for (let i = 0; i < peoples.length; i++) {
                ...
                if (flag == 1) {
                    ...
                    let html = '<div class="chat_list" style="cursor: pointer;" id="' + user_name + '">' +
                        ...
                        '<div class="chat_img"> <img id="' + user_name + '-img" src="../resource/avtar/' + peoples[i].avtar + '" alt="' + user_name + '"></div>' +
                        '<div class="chat_ib">' +
        ...
        for (let i = 0; i < msg.length; i++) {
            ...
            if (msg[i][0] == 'o') {
                html = '<div class="outgoing_msg">' +
                    '<div class="sent_msg">' +
                    '<p>' + msg[i][1] + ' <br> <a style="color:#ecf511;" href="../resource/msg_file/' + msg[i][2] + '" target="_blank">' + msg[i][3] + '</a> </p>' +
                ...
                html = '<div class="incoming_msg mt-1">' +
                    ...
                    '<p>' + msg[i][1] + ' <br> <a href="../resource/msg_file/' + msg[i][2] + '" target="_blank">' + msg[i][3] + '</a> </p>' +
                    // '<span class="time_date"> 11:01 AM    |    Today</span></div>' +
                    '</div>' +
                    '</div>';
            }
    ...
    $(document).on("click", ".chat_list", function() {
        ...
        $.ajax({
            url: ajax_url + 's/get_messages_of_the_user',
            type: 'post',
            data: { id: current_id },
            dataType: 'json',
    ...
    $('#send_msg_to_man').on('submit', function(e) {
        ...
        $.ajax({
            ...
            url: ajax_url + 's/send_msg_to_man',
            type: 'post',
            data: formData,
            processData: false,
            contentType: false,
    ...
    $('#custom_order_request').on('submit', function(e) {
        ...
        $.ajax({
            ...
            url: ajax_url + 's/send_custom_order',
            type: 'post',
            data: formData,
            processData: false,
            contentType: false,
